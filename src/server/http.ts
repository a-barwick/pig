...
export async function serve(
  ...
  const server = createServer(async (req, res) => {
    ...
    const requestOrigin =
      req.headers.host === localhostHost ? localhostOrigin : origin;
    if (
    ...
    if (path.startsWith("/trpc/")) {
      if (!authorized(req, requestOrigin, token)) {
        res.writeHead(401);
        res.end("Open the workspace page first.");
        return;
      }
    ...
    res.setHeader(
      "Set-Cookie",
      `pi_local=${token}; HttpOnly; SameSite=Strict; Path=/`,
    );
