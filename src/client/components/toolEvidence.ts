...
export function toolEvidence(call: ToolCall) {
  ...
  const diff = typeof details.diff === 'string' ? details.diff : typeof result.diff === 'string' ? result.diff : undefined;
  const code = result.exitCode ?? details.exitCode;
  const exitCode = typeof code === 'number' || typeof code === 'string' ? code : undefined;
  const blocks = Array.isArray(result.content) ? result.content : [];
  const output = blocks.map(record).filter(block => block.type === 'text' && typeof block.text === 'string').map(block => block.text as string);
  ...
  const textStatus = call.name === 'bash' && call.status === 'failed'
