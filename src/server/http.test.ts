...
let cookie: string;
const runtime = {snapshot:()=>null,dispose:async()=>{},async *events(signal?:AbortSignal){yield {cursor:1,sessionId:'test',kind:'accepted',requestId:'test'};await new Promise<void>(r=>signal?.addEventListener('abort',()=>r(),{once:true}));}} as unknown as RuntimeService;
beforeAll(async()=>{app=await serve(runtime,{} as ConfigService,14317,true);const page=await fetch(app.origin);cookie=page.headers.get('set-cookie')!.split(';')[0];});
...
describe('loopback RPC and SSE boundary',()=>{
 ...
 const controller=new AbortController();const r=await fetch(app.origin+'/trpc/events',{headers:{cookie},signal:controller.signal});
 expect(r.headers.get('content-type')).toContain('text/event-stream');const reader=r.body!.getReader();let text='';
 while(!text.includes('test')){const chunk=await reader.read();if(chunk.done)break;text+=new TextDecoder().decode(chunk.value);}
 expect(text).toContain('accepted');expect(text).toContain('requestId');controller.abort();
 });
});
