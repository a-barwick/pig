...
const model = {
  provider: actual.defaultProvider ?? "openai-codex",
  id: actual.defaultModel ?? "gpt-5.6-sol",
};
...
function calls(s: Snapshot) {
  return s.messages.flatMap((m) => m.toolCalls ?? []);
}
try {
  const content =
    "---\nname: dashboard-smoke\ndescription: Explicit dashboard acceptance skill.\n---\nReply with SKILL_REVISION_ONE.\n";
  let saved = await config.saveSkill({
    projectPath,
    name: "dashboard-smoke",
    scope: "project",
  ...
  let state = await runtime.open({ projectPath });
  state = await settled();
  assert(!state.trust.trusted);
  assert(
    state.resources.some((r) => r.kind === "skill" && !r.loaded && r.reason),
  );
  report.untrustedResources = true;
  await runtime.trust(projectPath);
  await settled();
  state = await runtime.open({ projectPath });
  state = await settled();
  assert(state.resources.some((r) => r.kind === "skill" && r.loaded));
  let settings = await config.settings({ projectPath, scope: "project" });
  let settingSave = await config.saveSettings({
    projectPath,
    scope: "project",
  ...
  settingSave = await config.saveSettings({
    projectPath,
    scope: "project",
    expectedRevision: settings.revision,
    thinking: null,
  ...
  state = await settled();
  assert(calls(state).some((c) => c.status === "completed"));
  assert(calls(state).some((c) => c.status === "failed"));
  assert(calls(state).every((c) => state.tools.some((t) => t.name === c.name)));
  report.tools = calls(state).map((c) => ({
    name: c.name,
    status: c.status,
    args: !!c.args,
    result: !!c.result,
  ...
  while (!calls(runtime.snapshot()!).some((c) => c.status === "running")) {
    assert(Date.now() < deadline, "Sleep tool did not start");
    await new Promise((r) => setTimeout(r, 100));
  }
  await runtime.steer({
    sessionId: state.sessionId,
  ...
  await runtime.stop(state.sessionId);
  state = await settled();
  ...
  runtime = await createRuntimeService(options);
  state = await runtime.open({ projectPath, sessionPath });
  state = await settled();
  assert.equal(state.messages.length, count);
  assert.equal(state.status, "idle");
  report.resumeAfterRuntimeRestart = true;
  let skill = await config.readSkill({ projectPath, resourceId });
  await writeFile(resourceId, content + "\nExternal change\n");
  const conflict = await config.saveSkill({
    projectPath,
    resourceId,
    name: skill.name,
    scope: "project",
  ...
  skill = await config.readSkill({ projectPath, resourceId });
  saved = await config.saveSkill({
    projectPath,
    resourceId,
    name: skill.name,
    scope: "project",
  ...
  assert.equal(state.trial?.revision, saved.revision);
  assert(
    state.messages.some(
      (m) => m.role === "assistant" && m.text.includes("SKILL_REVISION_TWO"),
    ),
  );
