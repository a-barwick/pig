...
const revision = (bytes: string | null) =>
  createHash("sha256")
    .update(bytes === null ? "\0missing" : bytes)
    .digest("hex");
...
const thinking = (value: unknown): Thinking | undefined =>
...
function safePath(base: string, target: string): string {
  ...
  if (!inside(target, base))
    throw new Error("Path is outside the allowed resource directory.");
  // Include base ancestors: a swapped .pi/skills or settings symlink must never grant access.
  let cursor = target;
  while (true) {
    try {
      if (lstatSync(cursor).isSymbolicLink())
        throw new Error("Symbolic links are not editable resources.");
    } catch (error) {
...
function json(bytes: string | null): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error(
      "Settings JSON is invalid. Fix it outside the app before saving.",
    );
  }
...
const model = (value: Record<string, unknown>): ModelInfo | undefined =>
  ...
  typeof value.defaultModel === "string"
    ...
    : undefined;
const thinkingMap = (value: unknown): Record<string, Thinking> =>
  Object.fromEntries(
    Object.entries(object(value)).filter(
      ([, level]) => thinking(level) !== undefined,
    ),
  ) as Record<string, Thinking>;
...
export function createConfigService(
  ...
  const undoRecords = new Map<
    string,
    { before: string | null; revision: string; base: string }
  >();
  function project(path: string) {
    ...
    if (!lstatSync(result).isDirectory())
      throw new Error("Project path must be a directory.");
    return result;
  }
  function baseFor(cwd: string, scope: Scope) {
    return scope === "global" ? agentDir : project(cwd);
  }
  function settingsPath(cwd: string, scope: Scope) {
    const base = baseFor(cwd, scope);
    return safePath(
      base,
      scope === "global"
  ...
  function roots(
    cwd: string,
  ): { base: string; path: string; scope: Scope; file?: boolean }[] {
    const p = project(cwd);
    const result: {
      base: string;
      path: string;
    ...
    for (const scope of ["global", "project"] as const) {
      ...
      for (const item of Array.isArray(settings.skills)
        ...
        if (!raw) continue;
        const path = resolve(
          dirname(settingsPath(cwd, scope)),
  ...
  function paths(cwd: string) {
    ...
    for (const root of roots(cwd)) {
      function walk(path: string, top: boolean) {
        ...
        if (lstatSync(path).isFile()) {
          if (path.endsWith(".md")) found.set(path, root);
          return;
        }
        const skill = join(path, "SKILL.md");
        if (existsSync(skill)) {
          safePath(root.base, skill);
          found.set(skill, root);
          return;
        }
        for (const entry of readdirSync(path, { withFileTypes: true })) {
          ...
          else if (top && entry.isFile() && entry.name.endsWith(".md"))
            found.set(join(path, entry.name), root);
        }
      }
      try {
        walk(root.path, true);
      } catch (error) {
        if ((error as Error).message.includes("Symbolic links")) continue;
        throw error;
      }
    }
    return found;
  ...
  function skill(cwd: string, resourceId: string): SkillFile {
    ...
    if (!allowed)
      throw new Error("Skill is not an allowed discovered resource.");
    ...
    return {
      ...
      name:
        typeof frontmatter.name === "string"
          ? frontmatter.name
          : dirname(resourceId).split(sep).pop()!,
      description:
  ...
  function write(
    ...
    if (expected === null ? before !== null : revision(before) !== expected)
      return {
        ok: false,
        conflict: true,
        message:
          "This file changed outside the workbench. Your draft is preserved. Reload the saved file before saving again.",
    ...
    if (content === null) {
      ...
      try {
        writeFileSync(temp, content, {
          flag: "wx",
          mode: before === null ? 0o600 : lstatSync(path).mode & 0o777,
        });
        safePath(base, path);
        if (revision(read(path)) !== revision(before))
          return {
            ok: false,
            conflict: true,
            message: "File changed during save. Draft preserved.",
            currentRevision: revision(read(path)),
          };
  ...
  const guarded = (fn: () => SaveResult): SaveResult => {
    try {
      return fn();
    } catch (error) {
      return {
  ...
  return {
    async skills(cwd) {
      return [...paths(cwd).keys()].map((path) => skill(cwd, path));
    },
    async readSkill(input) {
      return skill(input.projectPath, input.resourceId);
    },
    async saveSkill(input) {
      return guarded(() => {
        ...
        if (
          ...
          throw new Error(
            "Skill frontmatter needs a lowercase hyphenated name (up to 64 characters).",
          );
        if (
          typeof frontmatter.description !== "string" ||
          ...
          throw new Error(
            "Skill needs a description (up to 1024 characters) and instructions.",
          );
        let path: string;
        let base: string;
        if (input.resourceId) {
          const allowed = paths(input.projectPath).get(input.resourceId);
          if (!allowed || allowed.scope !== input.scope)
            throw new Error("Skill is not an allowed resource in this scope.");
          path = input.resourceId;
          base = allowed.base;
        } else {
          if (input.expectedRevision !== null)
            throw new Error("New skills require a create-only revision.");
          if (input.name !== frontmatter.name)
            throw new Error("New skill name must match its frontmatter.");
          base = baseFor(input.projectPath, input.scope);
          path = join(
            base,
            input.scope === "global" ? "skills" : ".pi/skills",
            frontmatter.name,
            "SKILL.md",
          );
        }
    ...
    async undo(input) {
      return guarded(() => {
        ...
        const settingsAllowed = ["project", "global"].some(
          (scope) =>
            settingsPath(input.projectPath, scope as Scope) === input.path,
        );
        if (!settingsAllowed && !paths(input.projectPath).has(input.path))
          throw new Error(
            "Undo target is outside this project’s allowed resources.",
          );
        if (record.revision !== input.expectedRevision)
          return {
            ok: false,
            conflict: true,
            message: "Undo revision does not match the last save.",
          };
        const result = write(
          record.base,
          input.path,
    ...
    async settings(input): Promise<SettingsView> {
      ...
      const local = json(bytes);
      const global = json(read(settingsPath(input.projectPath, "global")));
      const effective =
        input.scope === "project" ? { ...global, ...local } : local;
      const mergedLevels = {
        ...thinkingMap(global.modelThinkingLevels),
        ...thinkingMap(local.modelThinkingLevels),
      };
      const effectiveModel = model(effective);
      const key =
        effectiveModel && `${effectiveModel.provider}/${effectiveModel.id}`;
      const effectiveThinking =
        (key && mergedLevels[key]) ||
        thinking(effective.defaultThinkingLevel) ||
        "medium";
      const source =
        key && mergedLevels[key]
          ? `Per-model override (${Object.hasOwn(thinkingMap(local.modelThinkingLevels), key) ? input.scope : "global"}): ${key}`
          : thinking(local.defaultThinkingLevel)
            ? `${input.scope} default`
            : thinking(global.defaultThinkingLevel)
              ? "global default"
              : "pi built-in default";
      ...
      return {
        ...
        defaultProvider:
          typeof local.defaultProvider === "string"
            ? local.defaultProvider
            : undefined,
        defaultModel:
          typeof local.defaultModel === "string"
            ? local.defaultModel
            : undefined,
        model: model(local),
        thinking: thinking(local.defaultThinkingLevel),
        effectiveModel,
        effectiveThinking,
        source,
        modelThinkingLevels: thinkingMap(local.modelThinkingLevels),
        effectiveModelThinkingLevels: mergedLevels,
      };
    },
    async saveSettings(input) {
      return guarded(() => {
        const path = settingsPath(input.projectPath, input.scope);
        const settings = json(read(path));
        if (input.model !== undefined) {
          if (input.model === null) {
            delete settings.defaultProvider;
            delete settings.defaultModel;
          } else {
            settings.defaultProvider = input.model.provider;
            settings.defaultModel = input.model.id;
          }
        }
        if (input.thinking !== undefined) {
          if (input.thinking === null) delete settings.defaultThinkingLevel;
          else {
            if (!thinking(input.thinking))
              throw new Error("Invalid thinking level.");
            settings.defaultThinkingLevel = input.thinking;
          }
        }
        if (input.modelThinking !== undefined) {
          const { provider, id, level } = input.modelThinking;
          const overrides = { ...object(settings.modelThinkingLevels) };
          const key = `${provider}/${id}`;
          if (level === null) delete overrides[key];
          else {
            if (!thinking(level)) throw new Error("Invalid thinking level.");
            overrides[key] = level;
          }
          if (Object.keys(overrides).length)
            settings.modelThinkingLevels = overrides;
          else delete settings.modelThinkingLevels;
        }
        return write(
          baseFor(input.projectPath, input.scope),
          path,
          input.expectedRevision,
