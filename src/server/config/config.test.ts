...
beforeEach(() => {
  root = realpathSync(mkdtempSync(join(tmpdir(), "pi-config-")));
  project = join(root, "project");
  agent = join(root, "agent");
  mkdirSync(join(project, ".pi"), { recursive: true });
  mkdirSync(agent);
  service = createConfigService({ agentDir: agent });
});
...
describe("scoped pi settings", () => {
  it("preserves unrelated data and credentials on disk but exposes only safe fields, and removes overrides for inheritance", async () => {
    const global = {
      ...
      defaultModel: "model",
      apiKey: "secret-never-return",
      metadata: { keep: [1, 2] },
      modelThinkingLevels: { "test/other": "max" },
    };
    writeFileSync(join(agent, "settings.json"), JSON.stringify(global));
    const original = {
      theme: "custom",
      compaction: { enabled: false },
      extension: { credential: "also-private" },
    };
    ...
    expect(initial.effectiveThinking).toBe("high");
    expect(JSON.stringify(initial)).not.toContain("also-private");
    ...
    expect(
      JSON.parse(readFileSync(join(project, ".pi/settings.json"), "utf8")),
    ).toEqual({ ...original, defaultThinkingLevel: "medium" });
    const saved = await service.settings({
      projectPath: project,
      scope: "project",
    });
    expect(saved.effectiveThinking).toBe("medium");
    ...
    expect(
      (await service.settings({ projectPath: project, scope: "project" }))
        .effectiveThinking,
    ).toBe("high");
    expect(
  ...
  it("merges per-model thinking, preserves unknown entries, and uses per-model precedence above default", async () => {
    writeFileSync(
      ...
      JSON.stringify({
        ...
        defaultModel: "m",
        defaultThinkingLevel: "high",
        modelThinkingLevels: { "test/m": "max", "test/other": "low" },
      }),
    );
    writeFileSync(
      join(project, ".pi/settings.json"),
      JSON.stringify({
        modelThinkingLevels: { "future/model": { unknown: true } },
      }),
    );
    let view = await service.settings({
      projectPath: project,
      scope: "project",
    });
    await service.saveSettings({
    ...
    view = await service.settings({ projectPath: project, scope: "project" });
    expect(view.effectiveThinking).toBe("max");
    await service.saveSettings({
      projectPath: project,
      scope: "project",
      expectedRevision: view.revision,
      modelThinking: { provider: "test", id: "m", level: "low" },
    });
    view = await service.settings({ projectPath: project, scope: "project" });
    expect(view.effectiveThinking).toBe("low");
    await service.saveSettings({
      projectPath: project,
      scope: "project",
      expectedRevision: view.revision,
      modelThinking: { provider: "test", id: "m", level: null },
    });
    expect(
      (await service.settings({ projectPath: project, scope: "project" }))
        .effectiveThinking,
    ...
    expect(
      JSON.parse(readFileSync(join(project, ".pi/settings.json"), "utf8"))
        .modelThinkingLevels["future/model"],
    ).toEqual({ unknown: true });
  });
  it("rejects outside edits and can restore exact prior bytes with undo", async () => {
    const path = join(project, ".pi/settings.json");
    const before = '{ "theme": "x", "metadata": [1, 2] }\n';
    ...
    expect(readFileSync(path, "utf8")).toBe('{"outside": true}');
    writeFileSync(path, before);
...
describe("skill revisions and resource boundaries", () => {
  it("creates, reads, edits, detects outside edit on save and undo, and restores the previous revision", async () => {
    ...
    const [file] = await service.skills(project);
    ...
    const saved = await service.saveSkill({
      ...
      content: text("review", "Different instructions."),
    });
    if (!saved.ok) throw new Error(saved.message);
    ...
    expect(
      (await service.readSkill({ projectPath: project, resourceId: file.path }))
        .content,
    ).toBe(text());
  });
  it("undoing creation removes only the newly created file and create-only cannot overwrite", async () => {
    ...
    const first = await service.saveSkill(input);
    if (!first.ok) throw new Error(first.message);
    expect(await service.saveSkill(input)).toMatchObject({
      ok: false,
      conflict: true,
    });
    expect(
  ...
  it("rejects arbitrary paths, traversal names, symlink directories and settings symlinks", async () => {
    ...
    mkdirSync(outside);
    writeFileSync(join(outside, "SKILL.md"), text());
    await expect(
      service.readSkill({
        projectPath: project,
        resourceId: join(outside, "SKILL.md"),
      }),
    ...
    await expect(
      service.settings({ projectPath: project, scope: "project" }),
  ...
  it("supports explicit configured local skill resources without allowing unrelated siblings", async () => {
    const path = join(root, "configured.md");
    writeFileSync(path, text());
    writeFileSync(
      join(agent, "settings.json"),
      JSON.stringify({ skills: [path] }),
    );
    ...
    await expect(
      service.readSkill({ projectPath: project, resourceId: sibling }),
    ).rejects.toThrow("allowed");
  });
});
...
describe("failure recovery", () => {
  it("does not leak malformed JSON contents through reads", async () => {
    writeFileSync(
      join(agent, "settings.json"),
      '{"apiKey":"sensitive-value", broken',
    );
    await expect(
      service.settings({ projectPath: project, scope: "global" }),
    ).rejects.toThrow("Settings JSON is invalid");
    try {
      await service.settings({ projectPath: project, scope: "global" });
    } catch (error) {
  ...
  it("rejects a skill replaced with a symlink after discovery", async () => {
    ...
    writeFileSync(external, "keep untouched");
    rmSync(created.path);
    symlinkSync(external, created.path);
    expect(
      await service.saveSkill({
        projectPath: project,
        resourceId: created.path,
        name: "review",
        scope: "project",
        expectedRevision: created.revision,
        content: text("review", "overwrite"),
      }),
    ).toMatchObject({ ok: false });
    expect(
      await service.undo({
  ...
  it("inherits provider and model independently, and edits global settings without deleting unknown fields", async () => {
    ...
    expect(
      (await service.settings({ projectPath: project, scope: "project" }))
        .effectiveModel,
    ).toMatchObject({ provider: "test", id: "project-model" });
    ...
    expect(
      JSON.parse(readFileSync(join(project, ".pi/settings.json"), "utf8")),
    ).toEqual({ defaultModel: "project-model" });
  });
...
it("matches installed pi 0.85.1 model-thinking resolution for trusted project defaults", async () => {
  writeFileSync(
    ...
    JSON.stringify({
      ...
      defaultModel: "m",
      defaultThinkingLevel: "high",
      modelThinkingLevels: { "test/m": "max" },
    }),
  );
  writeFileSync(
    join(project, ".pi/settings.json"),
    JSON.stringify({
      ...
      modelThinkingLevels: { "test/other": "low" },
    }),
  );
  ...
  expect(view.effectiveThinking).toBe(
    sdk.getModelThinkingLevel("test", "m") ??
      sdk.getDefaultThinkingLevel() ??
      "medium",
  );
  expect(view.effectiveModelThinkingLevels).toEqual(
    sdk.getAllModelThinkingLevels(),
  );
});
...
it.each([
...
])("reports and removes partial scoped model defaults: %j", async (partial) => {
  const global = {
  ...
  writeFileSync(
    join(project, ".pi/settings.json"),
    JSON.stringify({ ...partial, unrelated: { keep: true } }),
  );
  const view = await service.settings({
    projectPath: project,
    scope: "project",
  ...
  expect(saved.ok).toBe(true);
  const after = await service.settings({
    projectPath: project,
    scope: "project",
  });
  expect(after.defaultProvider).toBeUndefined();
  expect(after.defaultModel).toBeUndefined();
  expect(after.effectiveModel).toMatchObject({
