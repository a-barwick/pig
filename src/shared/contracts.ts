...
export interface Resource {
  kind: "skill" | "extension" | "instruction" | "prompt";
  name: string;
  path: string;
  loaded: boolean;
  source: string;
...
export interface RuntimeEvent {
  cursor: number;
  sessionId: string;
  kind: "snapshot" | "error" | "accepted" | "cancel_requested";
  snapshot?: Snapshot;
  requestId?: string;
  message?: string;
}
...
export type SaveResult =
  | { ok: true; revision: string; path: string; applied: false }
  | { ok: false; conflict: boolean; message: string; currentRevision?: string };
export interface SettingsView {
  ...
  source: string;
  modelThinkingLevels?: Record<string, Thinking>;
  effectiveModelThinkingLevels?: Record<string, Thinking>;
}
...
export interface ConfigService {
  ...
  saveSettings(input: {
    ...
    modelThinking?: { provider: string; id: string; level: Thinking | null };
  }): Promise<SaveResult>;
}
